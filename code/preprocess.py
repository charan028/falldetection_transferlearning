...
def split_indexes(video_files):
    ...
    tt_count = 0
    idx1 = len(counts)  # default to all for train if no split
    idx2 = len(counts)  # default to all for val if no split
...
def fall_annotations(video_files):
    ...
    for file in video_files:
        annotation_file = file.replace('Videos', 'Annotation_files').replace('.avi', '.txt')
        print(f"Checking annotation file: {annotation_file}")  # Add this to check the path
        if not os.path.exists(annotation_file):
            print(f"Annotation file not found: {annotation_file}")  # Alert if missing
            continue
...
def prepare_train_val_test(location, src_dir):
    video_files = glob.glob(os.path.join(src_dir, location, '*.avi'))
    print(f"Searching in path: {os.path.join(src_dir, location, '*.avi')}")
    video_files = sorted(video_files)
    print(f"Found {len(video_files)} video files in {location}")
...
def create_MHI(data, dst, dataset='train'):
    ...
    for file_path, annotation in data: 
        ...
        print(f"Processing video: {file_path}")  # Add this to check if the video is being processed
        ...
        while(cap.isOpened()):
            ...
            if img is not None:
                if frame_id >= start_fall and frame_id <= stop_fall:
                    ...
                    print(f"Saved fall frame: {prefix}_{frame_id}.png")  # Add this to track saved frames
                else:
                    cv2.imwrite(f'{dest_path}/not_fall/{prefix}_{frame_id}.png', img)
                    print(f"Saved not_fall frame: {prefix}_{frame_id}.png")  # Add this to track saved frames
